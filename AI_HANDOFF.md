...
10. **Rate limiting** en `proxy.ts` + `lib/rateLimit.ts` (ventana deslizante in-memory, testeada): 300 lecturas y 60 escrituras por minuto por usuario (o IP anónima), respuesta 429 con `Retry-After`. Los límites de lectura son generosos a propósito: el polling de señales WebRTC durante una llamada ronda las 75 req/min.
...
- **Auth**: Clerk. `proxy.ts` protege todo excepto `/`, `/sign-in`, `/sign-up`, `/api/webhook`. Toda ruta API autenticada empieza por `lib/getOrCreateUser.ts` (auto-provisiona usuario en DB y migra clerkId dev→prod).
